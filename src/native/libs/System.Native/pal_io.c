...
// dirPath must remain valid for the life of the association
...
int32_t SystemNative_PortAssociate(intptr_t port, intptr_t pFileObj, char *dirPath, TimeSpec *mtime, uint32_t events, intptr_t cookie)
{
#if HAVE_SUNOS_PORTFS
    struct file_obj *fo;
    ...
    return port_associate(pfd, PORT_SOURCE_FILE, (uintptr_t)fo,
            events, (void *)cookie);
#else
...
// Send a "user" even to a port.  Used to cancel a PortGet call.
