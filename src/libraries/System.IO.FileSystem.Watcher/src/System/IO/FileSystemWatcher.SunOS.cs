...
namespace System.IO
{
    public partial class FileSystemWatcher
    {
        ...
        private sealed class RunningInstance
        {
            ...
            private const uint DirectoryEventMask = (uint)Interop.PortFs.PortEvent.FILE_MODIFIED;
            ...
            private readonly uint _portEventMask;  // For file entries, computed from NotifyFilters
                ...
                bool includeSubdirectories, NotifyFilters notifyFilters, CancellationToken cancellationToken)
            {
                ...
                if (_portfsHandle.IsInvalid)
                {
                    ...
                    {
                        Interop.Error.EMFILE => new IOException(SR.IOException_INotifyInstanceUserLimitExceeded, error.RawErrno),
                        Interop.Error.ENFILE => new IOException(SR.IOException_INotifyInstanceSystemLimitExceeded, error.RawErrno),
                        _ => Interop.GetExceptionForIoErrno(error)
                    };
                }
            ...
            private static uint GetPortEventMask(NotifyFilters filters)
            {
                uint mask = 0;
            ...
            private void AssociateDirectory()
            {
                ...
                unsafe
                {
                    fixed (byte* ptr = _directoryFileObjBuffer)
                    {
                        ...
                        {
                            Interop.ErrorInfo error = Interop.Sys.GetLastErrorInfo();
                            Exception exc = error.Error == Interop.Error.ENOSPC
                                ? new IOException(SR.IOException_INotifyWatchesUserLimitExceeded, error.RawErrno)
            ...
            private void CancellationCallback()
            {
                ...
                // Send synthetic event to wake up PortGet on this instance only
                Interop.PortFs.PortSend(_portfsHandle, (int)Interop.PortFs.PortEvent.FILE_NOFOLLOW, CancellationCookie);
            }
            ...
            internal void Cancel()
            {
                ...
                // Then cancel this instance by sending PortSend to wake up PortGet
                Interop.PortFs.PortSend(_portfsHandle, (int)Interop.PortFs.PortEvent.FILE_NOFOLLOW, CancellationCookie);
            }
            ...
            private void Cleanup()
            {
                ...
                if (_directoryFileObjBuffer != null)
                {
                    unsafe
                    {
                        fixed (byte* ptr = _directoryFileObjBuffer)
                ...
                foreach (var (cookie, watch) in _nameToWatchMap)
                {
                    unsafe
                    {
                        fixed (byte* ptr = watch.buffer)
